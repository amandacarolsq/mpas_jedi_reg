...
#BOP
#
# !SCRIPT: 1a_run_hdiag_var
#
# !DESCRIPTION:
#
# !CALLING SEQUENCE:
#     
#        ./1a_run_hdiag_var.bash ${EXPDIR}/proc ${NMLDIR} ${TBLDIR} ${EXEDIR} ${WRFBIN} ${LABELI} ${LABELF} ${EXP} ${RES} ${NNODES} ${NTASKSPN} ${AREA} ${MESH_DIR}
#
...
samplesUnbalancedDir=${RUNDIR}/samples            # input Dir

workdir=${hdiagVarDir}/vargroup1
...
background error:
  ...
  ensemble:
    members from template:
      template:
        <<: *memberConfig
        filename: ${samplesUnbalancedDir}/PTB_f48mf24_%iMember%.nc
      pattern: %iMember%
      start: 1
      zero padding: 3
      nmembers: 9
